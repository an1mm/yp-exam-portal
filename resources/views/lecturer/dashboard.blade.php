...
<div class="max-w-7xl mx-auto w-full p-6">
    ...
    <!-- Quick Actions -->
    <div class="flex flex-row gap-3 mt-12 mb-6">
        <a href="{{ route('lecturer.exams.create') }}" 
           class="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-all flex items-center gap-2">
        ...
        <a href="{{ route('lecturer.subjects.create') }}" 
           class="bg-white border border-gray-200 px-4 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-slate-50 hover:shadow-sm transition-all flex items-center gap-2">
            ...
            <div class="bg-gradient-to-r from-red-50 to-orange-50 border-2 border-red-200 rounded-xl shadow-sm">
                <div class="px-6 py-4 border-b border-red-200 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-red-800">Active Exams - Currently in Progress</h3>
                            <p class="text-xs text-red-600 mt-0.5">Current Time: {{ now()->setTimezone('Asia/Kuala_Lumpur')->format('d M Y, H:i:s') }} (Kuala Lumpur)</p>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 hover:bg-gray-50 transition-colors duration-200">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-500 mt-1">{{ $subject->code }} � {{ $subject->classes->pluck('name')->join(', ') ?? ($subject->schoolClass->name ?? 'No class') }}</p>
                            </div>
                        </div>
                    </div>
                    @endforeach
